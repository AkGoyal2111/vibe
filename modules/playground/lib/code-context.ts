/**
 * Pure, side-effect-free helpers for analysing code around the cursor.
 *
 * These were extracted from the `/api/code-completion` route so they can be
 * unit tested in isolation (no Next.js / network dependencies) and reused.
 */
...
/**
 * Builds a rich description of the code surrounding the cursor that can be fed
 * to an LLM for a contextually-aware completion.
 */
